...
class ValidMetric(MetricBase):
    ...
    def evaluate(self, score):
        ...
        #tf.math.argmax computes tf.math.maximum of elements across dimensions of a tensor

        self.top1_correct += int(tf.math.reduce_sum(tf.math.argmax(score, axis=1) == 0))
        self.top6_correct += int(tf.math.reduce_sum(tf.math.argmax(score, axis=1) <= 5))
        self.top10_correct += int(tf.math.reduce_sum(tf.math.argmax(score, axis=1) <= 9))

        # Fast ROUGE
        for i in range(batch_size):
            max_idx = int(tf.math.argmax(score[i], axis=0))
...
class MatchRougeMetric(MetricBase):
    ...
    def evaluate(self, score):
        ext = int(tf.math.argmax(score, axis=1)) # batch_size = 1
        self.ext.append(ext)
        self.cur_idx += 1
